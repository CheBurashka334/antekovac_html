</div><!-- /.workarea -->
<footer>
    <div class="footer">
        <div class="container">
            <div class="footer-links">
                <div class="footer-links__block">
                    <div class="footer-links__heading">
                        <h2>Обратная связь</h2>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Связаться с нами</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Отзывы</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Вопрос-ответ</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Контакты</a>
                    </div>
                </div>
                <div class="footer-links__block">
                    <div class="footer-links__heading">
                        <h2>Наши гарантии</h2>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Гарантии</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Устойчивость рисунка</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Ремонт изделий</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Качество</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Производство</a>
                    </div>
                </div>
                <div class="footer-links__block">
                    <div class="footer-links__heading">
                        <h2>Сотрудничество</h2>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Стать партнером</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Оптовым заказчикам</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Пиар и реклама</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Вакансии</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Творческие проекты</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Благотворительность</a>
                    </div>
                </div>
                <div class="footer-links__block">
                    <div class="footer-links__heading">
                        <h2>заказ</h2>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Оплата</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Доставка</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Индивидуальный заказ</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Возврат</a>
                    </div>
                </div>
                <div class="footer-links__block">
                    <div class="footer-links__heading">
                        <h2>каталог</h2>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Каталог товаров</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Товыры в наличии</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Все товары</a>
                    </div>
                    <div class="footer-links__link">
                        <a href="#">Коллекции</a>
                    </div>
                </div>
            </div>
            <div class="footer-copyrights">
                <div class="footer-copyrights__logo">
                    <span class="logo-box">
						<svg class="logo" role="img"><use xlink:href="#logo"/></svg>
					</span>
                    <span class="footer-copyrights__rights">
                        Все права защищены.<br>2007 - 2016
                    </span>
                </div>
            </div>
        </div>
    </div>
    <div class="footer-bottom">
        <div class="container">
            <div class="footer-bottom__wrapper">
                <div class="footer-bottom__phone">
                    8 800 700-75-99
                </div>
                <div class="footer-bottom__mail-us">
                    <a href="javascript:void(0)" class="btn btn--transparent btn--transparent--white btn--mail js-toggle-box" data-box=".page-aside.feed-back">Напишите нам</a>
                </div>
                <div class="footer-bottom__email">
                    order@antekovac.ru
                </div>
                <div class="footer-bottom__address">
                    москва, первый красносельский переулок, д.3
                </div>
                <div class="footer-bottom__socials">
                    <a href="" class="i-icon">
                        <i class="fa fa-youtube-play"></i>
                    </a>
                    <a href="" class="i-icon">
                        <i class="fa fa-instagram"></i>
                    </a>
                    <a href="" class="i-icon">
                        <i class="fa fa-facebook"></i>
                    </a>
                    <a href="" class="i-icon">
                        <i class="fa fa-vk"></i>
                    </a>
                </div>
            </div>
        </div>
    </div>
</footer>
<div class="modal search-form">
	<button class="btn btn--flat btn-close js-toggle-box" data-box=".modal.search-form"></button>
	<div class="modal__wrap">
		<div class="modal__inner">
			<div class="modal__heading"><h2>поиск по сайту</h2></div>
			<div class="search-form__field">
				<input type="text" class="transparent" />
				<button class="btn btn--flat--square--only-icon">
					<svg class="i-icon i-icon--stroke"><use xlink:href="#zoom"/></svg>
				</button>
			</div>
		</div>
	</div>
</div>
</main>

</body>

</html>
